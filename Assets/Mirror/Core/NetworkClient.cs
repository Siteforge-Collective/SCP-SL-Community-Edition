...
namespace Mirror
{
    ...
    public static partial class NetworkClient
    {
        ...
        static bool UnpackAndInvoke(NetworkReader reader, int channelId)
        {
            if (NetworkMessages.UnpackId(reader, out ushort msgType))
            {
                ...
                else
                {
                    ...
                    var info = MessageNameResolver.Resolve(msgType);

                    Debug.LogWarning(
                        $"[NetworkMessage] Unhandled message from {connection}:\n" +
                        $"  Type:    {info.FullName}\n" +
                        $"  Assembly: {info.AssemblyName}\n" +
                        $"  HashId:  {msgType}\n" +
                        $"  Status:  {(info.IsRegistered ? "REGISTERED but NO HANDLER" : "NOT REGISTERED at all")}\n" +
                        $"  Fix:     Check {info.AssemblyName} for RegisterHandler<{info.FullName}>() or [Command]/[ClientRpc] usage."
                    );
        ...
        internal static void ApplySpawnPayload(NetworkIdentity identity, SpawnMessage message)
        {
            ...
            // SetActive(true) above does NOT run Awake() if a parent is still
            // disabled (e.g. a facility room not yet activated by map generation),
            // so NetworkBehaviours can be null. GetComponentsInChildren(true) works
            // on inactive objects, so initialize now to keep every downstream path
            // (DeserializeClient, OnStartClient, OnObjectSpawnFinished) NRE-free.
        ...
        internal static void OnObjectSpawnFinished(ObjectSpawnFinishedMessage _)
        {
            ...
            foreach (NetworkIdentity identity in spawned.Values.OrderBy(uv => uv.netId))
            {
                ...
                if (identity != null)
                {
                    ...
                    // NetworkBehaviours can still be null for scene objects parented
                    // under a disabled object (e.g. a facility room not yet activated by
                    // map generation), so Awake() hasn't run. Initialize now so we don't
                    // NRE here and abort the whole spawn-finish handler (which would leave
                    // players/map half-spawned and trigger an endless round restart loop).
                    if (identity.NetworkBehaviours == null)
                        identity.InitializeNetworkBehaviours();
