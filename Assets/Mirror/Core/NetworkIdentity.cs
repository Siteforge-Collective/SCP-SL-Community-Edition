...
namespace Mirror
{
    ...
    public sealed class NetworkIdentity : MonoBehaviour
    {
        ...
        internal void HandleRemoteCall(byte componentIndex, ushort functionHash, RemoteCallType remoteCallType, NetworkReader reader, NetworkConnectionToClient senderConnection = null)
        {
            ...
            // NetworkBehaviours can still be null here if this is a scene object
            // parented under a disabled object (Awake hasn't run). Initialize it
            // now so an incoming RPC doesn't NRE.
        ...
        void ValidateComponents()
        {
            if (NetworkBehaviours == null)
            {
                // This is normal for scene objects parented under a still-disabled
                // object (e.g. a facility room not yet activated by map generation):
                // Awake() -> InitializeNetworkBehaviours() hasn't run yet, so the
                // array is null. GetComponentsInChildren(true) works on inactive
                // objects, so we can safely initialize it now instead of NRE'ing on
                // spawn/RPC deserialization. InitializeNetworkBehaviours sets the
                // array (non-null) before re-entering here, so there's no recursion.
                InitializeNetworkBehaviours();
            }
