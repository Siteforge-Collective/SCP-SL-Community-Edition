...
namespace Mirror.LiteNetLib4Mirror
{
	public static class LiteNetLib4MirrorClient
	{
		...
		{
			return LiteNetLib4MirrorCore.Host?.FirstPeer?.Ping ?? 0;
		}
        ...
        private static void OnNetworkError(IPEndPoint endpoint, SocketError socketerror)
        {
            ...
            TransportError mirrorError = TransportError.Unexpected;

            switch (socketerror)
            {
                case SocketError.HostNotFound:
                    mirrorError = TransportError.DnsResolve;
                    break;
                case SocketError.ConnectionRefused:
                    mirrorError = TransportError.Refused;
                    break;
                case SocketError.TimedOut:
                    mirrorError = TransportError.Timeout;
                    break;
                case SocketError.ConnectionReset:
                case SocketError.ConnectionAborted:
                    mirrorError = TransportError.ConnectionClosed;
                    break;
            }

            LiteNetLib4MirrorTransport.Singleton.OnClientError?.Invoke(mirrorError, socketerror.ToString());
