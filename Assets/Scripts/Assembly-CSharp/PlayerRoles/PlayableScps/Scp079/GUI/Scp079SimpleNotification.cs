...
namespace PlayerRoles.PlayableScps.Scp079.GUI
{
	public class Scp079SimpleNotification : IScp079Notification
	{
		...
		public virtual float Opacity => Mathf.Min(1f, 1f - (CurrentTime - _endTime) / FadeOutDuration);

		public float Height
		{
			get
			{
				float t = (Opacity > 0f) ? (Elapsed / FadeInTime) : (1f + Opacity);
				return Mathf.Lerp(InitialSize, _totalHeight, t);
			}
		}
		...
		public NotificationSound Sound
		{
			get
			{
				int num = Mathf.CeilToInt((_totalWritten - _lettersOffset) * SoundRateRatio);
				if (num <= _prevPlayed)
				{
					return NotificationSound.None;
				}
				_prevPlayed = num;
				return _mute ? NotificationSound.None : NotificationSound.Standard;
			}
		}
		...
		public Scp079SimpleNotification(string targetContent, bool mute = false)
		{
			...
			_totalHeight = HeightPerLine * (1 + _length - _targetContent.Replace("\n", string.Empty).Length);
			_startTime = CurrentTime;
			_endTime = _startTime + AbsoluteDuration + PerLetterDuration * _length;
		}

		private void WriteLetters()
		{
			int num = Mathf.RoundToInt((Elapsed - FadeInTime) * LettersPerSecond) + _lettersOffset;
			if (num <= 0)
			{
				return;
			}
