...
namespace PlayerRoles.PlayableScps.Scp079
{
    public class Scp079BlackoutZoneAbility : Scp079KeyAbilityBase, IScp079LevelUpNotifier
    {
        ...
        public override bool IsVisible
        {
            get
            {
                if (!Scp079Role.LocalInstanceActive || !Unlocked || Cursor.visible || !Scp079ToggleMapAbility.MapState)
                    return false;
        ...
        public override void ClientProcessRpc(NetworkReader reader)
        {
            ...
            _syncZone = (FacilityZone)reader.ReadByte();
            _cooldownTimer.ReadCooldown(reader);

            if (base.Owner == null || base.Owner.isLocalPlayer)
                return;
            if (SpectatorNetworking.IsLocallySpectated(base.Owner))
                return;

            GetSubroutine<Scp079BlackoutRoomAbility>(out var roomAbility);
            if (roomAbility == null)
                return;
            ...
            {
                if (instance.Room.Zone == _syncZone)
                    roomAbility.PlaySoundForController(instance);
            }
        }
    }
}
