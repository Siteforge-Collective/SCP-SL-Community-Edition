...
namespace CommandSystem.Commands.Console.Volume
{
    public class VoiceCommand : ICommand
    {
        ...
        public string Description { get; } = "Modifies the volume for voice chat.";
            ...
            if (float.TryParse(normalized, NumberStyles.Any, CultureInfo.InvariantCulture, out float value))
            {
                foreach (GameSettings.GameSettingsAudioSlider slider in GameSettings.singleton.AudioSliders)
                {
                    if (slider.Key == "Voice")
                    {
                        ...
                        response = string.Format("Set voice chat volume to {0}", value);
                        return true;
                    }
                }

                response = "Unable to set voice chat volume.";
                return false;
            }
