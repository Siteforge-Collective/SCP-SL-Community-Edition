...
public void OnEnable()
    ...
    private bool NotifyDecals()
    {
        ...
        if (decalsType != null)
        {
            ...
            DecalsEnabled = GameObject.FindFirstObjectByType(decalsType) != null && (GameObject.FindFirstObjectByType(decalsType) is MonoBehaviour) && (GameObject.FindFirstObjectByType(decalsType) as MonoBehaviour).enabled;
            if (DecalsEnabled)
            {
                (GameObject.FindFirstObjectByType(decalsType) as MonoBehaviour).Invoke("OnDisable", 0);
                (GameObject.FindFirstObjectByType(decalsType) as MonoBehaviour).Invoke("OnEnable", 0);
                return true;
            }
        }
        return false;
