        ...
        public override void OnInspectorGUI()
        {
            ...
            if (EditorGUILayout.BeginFadeGroup(m_showControls.faded))
            {
                ...
                else if (colorMode.value.intValue == 2) //Skybox
                {
                    ...
                    {
                        if (UnityEngine.Rendering.GraphicsSettings.defaultRenderPipeline.name.Contains("LWRP")) EditorGUILayout.HelpBox("This feature requires atleast LWRP version 5.7.2. If you're using this version, rerun the installer through the Help window", MessageType.Error);
                    }
#endif
                    if (UnityEngine.Rendering.GraphicsSettings.defaultRenderPipeline)
                    {
                        if (UnityEngine.Rendering.GraphicsSettings.defaultRenderPipeline.name.Contains("HDR")) EditorGUILayout.HelpBox("This feature is not supported in the HDRP", MessageType.Error);
                    }
