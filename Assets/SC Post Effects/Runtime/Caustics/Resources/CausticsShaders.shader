Shader "Hidden/SC Post Effects/Caustics"
{
	...
	float LinearDistanceFade(float3 worldPos, float startFade, float endFade, float offset, float maxFade)
	{
		float dist = length(worldPos - _WorldSpaceCameraPos.xyz) + offset;
		return saturate(1.0 - (dist - startFade) / max(endFade - startFade, 0.0001)) * maxFade;
	}
