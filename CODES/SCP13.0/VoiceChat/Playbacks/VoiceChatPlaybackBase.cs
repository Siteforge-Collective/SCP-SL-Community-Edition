...
namespace VoiceChat.Playbacks
{
    ...
    public abstract class VoiceChatPlaybackBase : MonoBehaviour
    {
        ...
        private static AudioClip Flatline
        {
            get
            {
                int num = 48000;
        ...
        {
            if (_collectedSamples >= LoudnessCollectorThreshold)
            {
                float num = _collectedLoudness * LoudnessCollectorMultiplier;
                _targetLoudness = Mathf.Sqrt(num / (float)_collectedSamples);
                _collectedLoudness = 0f;
                _collectedSamples = 0;
            }
            Loudness = Mathf.Lerp(Loudness, _targetLoudness, Time.deltaTime * LoudnessLerpSpeed);
        }
