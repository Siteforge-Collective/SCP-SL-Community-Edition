...
public class StartScreen : MonoBehaviour
{
    ...
    {
        bool useFastFade = true;//UserSettings.UserSetting<bool>.Get<UserSettings.UserInterfaceSettings.UISetting>();
        _limiter = useFastFade ? _fadeLimitFast : _fadeLimitNormal;

        _length = _limiter.length > 0 ? _limiter.keys[_limiter.length - 1].time : 3f;

        _roleDescText.text = TranslationReader.Get("Class_Descriptions", 0, prb.RoleName);
